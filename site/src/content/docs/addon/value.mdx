...
tags: [addon, value, value-extractor]
toc: true
---
...
<Callout type="info">
  Attribute tips: - `@aria-checked` extracts the `aria-checked` attribute value (e.g., `true`/`false`). - `@class`
  extracts the class string (e.g., `custom-checkbox`). Consider a regex like `(^|\s)token(\s|$)` (using single
  backslashes for literal regex) to test a single class. - `@type` extracts the `type` attribute (e.g., `checkbox`). -
