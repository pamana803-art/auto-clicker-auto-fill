  ...
  return (
    <div className='rounded sidebar bg-body'>
      <div className='d-flex justify-content-between align-items-center border-bottom p-2 rounded-top'>
        <button className="btn" variant='primary' onClick={() => dispatch(addConfig())} data-testid='add-configuration'>
      ...
      <ListGroup as='ul' data-testid='configuration-list'>
        {configs.length === 0 && <li className="list-group-item" as='li'>{t('configuration.noConfig')}</li>}
        {configs.map((config, index) => (
          ...
          >
            ...
            <button className="btn" variant='link' data-testid='remove-configuration' onClick={(e) => onRemoveConfig(e, config)} disabled={configs.length === 1}>
