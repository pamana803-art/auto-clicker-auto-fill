import React, { ChangeEvent, Dispatch, SetStateAction, forwardRef, useEffect, useImperativeHandle, useRef, useState } from 'react';

import PropTypes from 'prop-types';
import { Button, Col, Form, Modal, Row, Table } from 'react-bootstrap';
import { useTranslation } from 'react-i18next';
import { ACTION_CONDITION_OPR, ACTION_RUNNING, Action, ActionCondition, ActionStatement, Configuration } from '@dhruv-techapps/acf-common';
...
type ActionConditionModalRef = {
  showCondition: (index: number, actions: Array<Action>, statement: ActionStatement) => void;
};

type ActionConditionModalProps = {
  configIndex: number;
  setConfigs: Dispatch<SetStateAction<Array<Configuration>>>;
};

const ActionConditionModal = forwardRef<ActionConditionModalRef, ActionConditionModalProps>(({ configIndex, setConfigs }, ref) => {
  const { t } = useTranslation();
  const [show, setShow] = useState(false);
  const [conditions, setConditions] = useState<Array<ActionCondition>>([]);
  const [then, setThen] = useState<ACTION_RUNNING>(ACTION_RUNNING.PROCEED);
  const [goto, setGoto] = useState<number | undefined>(-1);
  const [message, setMessage] = useState<string | undefined>();
  const actionIndex = useRef(-1);
  const actions = useRef<Array<Action>>([]);
  const allActions = useRef<Array<Action>>([]);
  const updateRef = useRef(false);

  const onUpdate = (e: React.MouseEvent<HTMLInputElement>, index: number) => {
  ...
  const onUpdateGoto = (e: ChangeEvent<HTMLSelectElement>) => {
    updateRef.current = true;
    setGoto(Number(e.currentTarget.value));
  };
  ...
  const onReset = () => {
    updateRef.current = true;
    setConditions([]);
    setThen(ACTION_RUNNING.PROCEED);
    setGoto(-1);
    handleClose();
  };

  useImperativeHandle(ref, () => ({
    showCondition(index, _actions, statement ) {
  ...
  const addCondition = () => {
    //TODO setConditions((_conditions) => [..._conditions, { operator: ACTION_CONDITION_OPR.AND }]);
  };
        ...
        <Table role="table" className="mb-0">
          ...
          <tbody>
            {conditions.map((_condition, index) => (
              <ActionAndStatus key={index} actions={actions.current} condition={_condition} conditionIndex={index} setConditions={setConditions} updateRef={updateRef} />
            ))}
          </tbody>
        </Table>
